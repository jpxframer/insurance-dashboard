 ...
 * alone as cards. Neither frame carries the dashboard's greeting bar, and the
 * desktop list starts at the top of the viewport, so the page passes
 * `topBar={null}` and owns everything right of the sidebar.
 *
 * The index opens the first message on desktop, which is the state the frame
 * draws. Every row and card links to `/inbox/[id]` from there.
 */
export default function InboxPage() {
  return (
    ...
    >
      <InboxDesktop selectedId={inboxMessages[0].id} />
      <InboxCardList />
    </DashboardShell>
  );
}
